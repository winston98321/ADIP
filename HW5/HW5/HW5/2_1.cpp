...
cv::Mat createGaussianFilter(int rows, int cols, double D0, bool isLowPass) {
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            double value = exp(-0.5 * (distance * distance) / (D0 * D0));

            if (isLowPass) {
                filter.at<double>(i, j) = value;
            }
            else {
                filter.at<double>(i, j) = 1.0 - value;
            }
        }
    }
...
int main() {
    ...
    for (int shi = 0; shi < 2; shi++) {
        for (double D0 : D0_values) {
            ...
            cv::Mat lowPassFilter = createGaussianFilter(rows, cols, D0, true);

            // Create Gaussian high-pass filter
            cv::Mat highPassFilter = createGaussianFilter(rows, cols, D0, false);
            ...
            if (shi == 0) {
                imwrite("image_file/gaussian/ORIGIN_LPF_D0_" + std::to_string(D0) + "_SPRCTRUM.png", lowPassFilter);
                imwrite("image_file/gaussian/ORIGIN_HPF_D0_" + std::to_string(D0) + "_SPRCTRUM.png", highPassFilter);
            }
            else {
                imwrite("image_file/gaussian/LPF_D0_" + std::to_string(D0) + "_SPRCTRUM.png", lowPassFilter);
                imwrite("image_file/gaussian/HPF_D0_" + std::to_string(D0) + "_SPRCTRUM.png", highPassFilter);
            }
            // Convert the results to 8-bit unsigned integer type
            lowPassResult.convertTo(lowPassResult, CV_8U,255);
            highPassResult.convertTo(highPassResult, CV_8U,1);



            // Display the filtered images
            if (shi == 0) {
                imwrite("image_file/gaussian/ORIGIN_LPF_D0_" + std::to_string(D0) + "_IMG.png", lowPassResult);
                imwrite("image_file/gaussian/ORIGIN_HPF_D0_IMG" + std::to_string(D0) + "_IMG.png", highPassResult);
            }
            else {
                imwrite("image_file/gaussian/LPF_D0_" + std::to_string(D0) + "_IMG.png", lowPassResult);
                imwrite("image_file/gaussian/HPF_D0_" + std::to_string(D0) + "_IMG.png", highPassResult);
            }
