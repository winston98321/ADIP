...
cv::Mat createIdealFilter(int rows, int cols, double D0, bool isLowPass) {
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            double value;
            if ((isLowPass && distance <= D0) || (!isLowPass && distance > D0)) {
                value = 1.0;
            }
            else {
                value = 0.0;
    ...
    for (int shi = 0; shi < 2; shi++) {
        for (int D0 : D0_values) {
            for (int n : {1, 2, 3, 4}) {
                ...
                cv::Mat lowPassFilter = createIdealFilter(rows, cols, D0, true);


                // Create Gaussian high-pass filter
                cv::Mat highPassFilter = createIdealFilter(rows, cols, D0, false);
                ...
                if (shi == 0) {
                    imwrite("image_file/idealfilter/ORIGIN_LPF_D0_" + std::to_string(D0) +"_SPRCTRUM.png", lowPassFilter);
                    imwrite("image_file/idealfilter/ORIGIN_HPF_D0_" + std::to_string(D0) + "_SPRCTRUM.png", highPassFilter);
                }
                else {
                    imwrite("image_file/idealfilter/LPF_D0_" + std::to_string(D0) +"_SPRCTRUM.png", lowPassFilter);
                    imwrite("image_file/idealfilter/HPF_D0_" + std::to_string(D0) + "_SPRCTRUM.png", highPassFilter);
                }
                ...
                if (shi == 0) {
                    imwrite("image_file/idealfilter/ORIGIN_Idealfilter_LPF_D0_" + std::to_string(D0) + ".png", lowPassResult);
                    imwrite("image_file/idealfilter/ORIGIN_Idealfilter_HPF_D0_" + std::to_string(D0) + ".png", highPassResult);
                }
                else {
                    imwrite("image_file/idealfilter/Idealfilter_LPF_D0_" + std::to_string(D0)  + ".png", lowPassResult);
                    imwrite("image_file/idealfilter/Idealfilter_HPF_D0_" + std::to_string(D0) + ".png", highPassResult);
                }
